...
def prep_output_matches_expected(csv_path,
                                 df_returned_by_prep):
    ...
    for column in vak.io.spect.DF_COLUMNS:
        if column == 'duration':
            ...
            check_exact = True
        assert_series_equal(df_from_csv_path[column], df_returned_by_prep[column], check_exact=check_exact)
...
def test_prep(config_type,
    ...
    vak_df, csv_path = vak.core.prep(data_dir=cfg.prep.data_dir,
                                     purpose=purpose,
    ...
    assert prep_output_matches_expected(csv_path,
                                        vak_df)
