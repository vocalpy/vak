...
SPLITS_JSON = {
    "splits_csv_path": "splits/inputs-targets-paths-csvs/replicate-1.splits.csv",
    "sample_id_vec_path": {
        "test": "splits/sample-id-vectors/test.sample_ids.npy",
        "train": "splits/sample-id-vectors/train.sample_ids.npy",
        "val": "splits/sample-id-vectors/val.sample_ids.npy"
    },
    "inds_in_sample_vec_path": {
        "test": "splits/inds-in-sample-vectors/test.inds_in_sample.npy",
        "train": "splits/inds-in-sample-vectors/train.inds_in_sample.npy",
        "val": "splits/inds-in-sample-vectors/val.inds_in_sample.npy"
    }
}
...
def mock_biosoundsegbench_dataset(tmp_path):
    ...
    splits_csv = df.to_csv(inputs_targets_csv_dir / "replicate-1.splits.csv")
    ...
    for split in "train", "val", "test":
        ...
        np.save(sample_id_vecs_dir / f"{split}.sample_ids.npy", sample_id_vec)
        inds_in_sample_vec = np.arange(10)
        np.save(inds_in_sample_vecs_dir / f"{split}.inds_in_sample.npy", inds_in_sample_vec)

    splits_path = dataset_path / "split-replicate-1.json"
