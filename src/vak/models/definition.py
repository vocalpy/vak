...
VALID_CONFIG_KEYS = REQUIRED_MODEL_DEFINITION_CLASS_VARS[:-1]  # everything but 'default_config'
...
def validate(definition: Type[ModelDefinition]) -> Type[ModelDefinition]:
    ...
    if not all([expected_class_var_name in definition_class_var_names
                for expected_class_var_name in REQUIRED_MODEL_DEFINITION_CLASS_VARS]):
        missing_var_name = set(REQUIRED_MODEL_DEFINITION_CLASS_VARS) - set(definition_class_var_names)
        raise ValueError(
    ...
    if any([modeldef_var_name not in REQUIRED_MODEL_DEFINITION_CLASS_VARS
            for modeldef_var_name in definition_class_var_names]):
        extra_var_name = set(definition_class_var_names) - set(REQUIRED_MODEL_DEFINITION_CLASS_VARS)
        raise ValueError(
    ...
    for metrics_dict_key, metrics_dict_val in metrics_obj.items():
        ...
        if not (inspect.isclass(metrics_dict_val) and callable(metrics_dict_val)):
            raise TypeError(
    ...
    if network_config is None:
        if inspect.isclass(definition.network):
            ...
            definition.default_config['network'] = {
                network_name: {}
                for network_name in definition.network.keys()
            }
        ...
        if inspect.isclass(definition.network):
            ...
            if any([
                ...
                invalid_keys = set(network_config.keys()) - set(network_init_params)
                raise ValueError(
            ...
            if any([network_name not in definition.network.keys()
                    for network_name in network_config.keys()]):

                invalid_network_names = [network_name
                                         for network_name in network_config.keys()
    ...
    if loss_config is None:
        ...
        if any(
            ...
            invalid_loss_kwargs = set(loss_config.keys()) - set(loss_init_params)
            raise ValueError(
                f'The following keyword arguments specified in the ``default_config`` '
    ...
    if optimizer_config is None:
        ...
        if any(
            ...
            invalid_optimizer_kwargs = set(optimizer_config.keys()) - set(optimizer_init_params)
            raise ValueError(
                f'The following keyword arguments specified in the ``default_config`` '
    ...
    if metrics_config is None:
        ...
        if any(
            ...
            invalid_metric_names = set(metrics_config.keys()) - set(definition.metrics.keys())
            raise ValueError(
        ...
        for metric_name, metric_class_config in metrics_config.items():
            ...
            if any(
                ...
                invalid_metric_class_kwargs = set(metric_class_config.keys()) - set(metric_class_init_params)
                raise ValueError(
                    f"The following keyword arguments specified in the ``default_config`` "
