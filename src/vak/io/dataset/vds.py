...
class Dataset:
    ...
    def all_voc(self, attribute, value):
        if not all([type(element) == Vocalization for element in value]):
            raise TypeError(f'all elements in voc_list must be of type vak.io.Vocalization')
    ...
    def load_spects(self,
        ...
        Returns
        -------
        vds : vak.io.VocalizationDataset
            new instance of io, with spectrogram loaded into metaspect attribute
            for each Vocalization in set

        Examples
        --------
        >>> vds = vak.io.spect.from_files(**from_files_kwargs)
    ...
    def move_spects(self, new_root, spect_dir_str='spectrograms_generated'):
        ...
        spect_dir_str : str
            ...
            spectrograms generated by vak.io.prep
