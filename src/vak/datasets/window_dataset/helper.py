...
def _vectors_from_df(
    ...
    *without* removing the elements from ``window_inds`` that are set to
    ...
    for ind, to_do_item in to_do:
        ...
        spect_dict = files.spect.load(source_path)
...
def vectors_from_csv_path(
        ...
        timebins_key: str = "t",
        crop_dur: float = None,
        timebin_dur: int = None,
        labelmap: dict | None = None,
) -> tuple[npt.NDArray, npt.NDArray, npt.NDArray]:
    """Get ``source_ids`` and ``spect_ind_vector`` from a
    ...
    window_inds : numpy.ndarray
        Starting indices of valid windows.
        This will always be smaller than ``source_ids``
        after the cropping now set to WindowDataset.INVALID_WINDOW_VAL
        so they will be removed
    """
    # pre-conditions handled by ``vectors_from_df`` are not repeated here
