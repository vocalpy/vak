...
class ED_TCN(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.cnn = torch.nn.Sequential(
            ...
            torch.nn.MaxPool2d(kernel_size=pool1_size,
                               stride=pool1_stride),
            Conv2dTF(in_channels=conv2d_1_filters,
                     out_channels=conv2d_2_filters,
                     kernel_size=conv2d_2_kernel_size,
                     padding=padding,
                     ),
            torch.nn.ReLU(inplace=True),
            torch.nn.MaxPool2d(kernel_size=pool2_size,
                               stride=pool2_stride),
        )
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Conv1d(self.n_cnn_features_out, conv1d_1_filters, conv1d_kernel_size, padding='same'),
            torch.nn.Dropout1d(p=0.3),
            NormReLU(),
            torch.nn.MaxPool1d(kernel_size=2),
            torch.nn.Conv1d(conv1d_1_filters, conv1d_2_filters, conv1d_kernel_size, padding='same'),
        ...
        self.decoder = torch.nn.Sequential(
            ...
            torch.nn.Conv1d(conv1d_2_filters, conv1d_2_filters, conv1d_kernel_size, padding='same'),
            torch.nn.Dropout1d(p=0.3),
            NormReLU(),
            torch.nn.Upsample(scale_factor=2),
            torch.nn.Conv1d(conv1d_2_filters, conv1d_1_filters, conv1d_kernel_size, padding='same'),
            torch.nn.Dropout1d(0.3),
            NormReLU(),
        )

        self.fc = torch.nn.Linear(in_features=conv1d_1_filters, out_features=self.num_classes)
    ...
    def forward(self, x):
        ...
        x = self.encoder(x)
        x = self.decoder(x)
        x = x.permute(0, 2, 1)  # so that we can project features down on to number of classes
        x = self.fc(x)
        x = x.permute(0, 2, 1)  # switch back to (batch, classes, time) for loss function
        return x
