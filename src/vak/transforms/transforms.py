...
class StandardizeSpect:
    ...
    @classmethod
    def fit_csv_path(cls, dataset_csv_path, split='train', spect_key="s"):
        """Returns a StandardizeSpect instance fit to a dataset,
        given the path to the csv file representing that dataset.

        Parameters
        ----------
        dataset_csv_path : str or pathlib.Path
            Path to a csv file representing a dataset.
            Csv file is generated by ``vak prep`` in the root of
        ...
        spect = files.spect.load(dataset_path / spect_paths[0])[spect_key]
        ...
        for spect_path in spect_paths[1:]:
            spect = files.spect.load(dataset_path / spect_path)[spect_key]
            mean_freqs += np.mean(spect, axis=1)
            std_freqs += np.std(spect, axis=1)
