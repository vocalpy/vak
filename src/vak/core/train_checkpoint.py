...
def train_checkpoint(
    ...
    """train models using training set specified in config.toml file from a checkpoint.
    ...
    for model_name, model in models_map.items():
        ...
        ## makes a new folder so as to not overwrite the old checkpoint
        results_model_root = results_path.joinpath(model_name)
        results_model_root.mkdir()
        ckpt_root = results_model_root.joinpath("checkpoints")
        ckpt_root.mkdir()
        log_or_print(f"training from checkpoint {model_name}", logger=logger, level="info")
