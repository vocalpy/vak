...
def learning_curve(
    ...
    learncurve_splits_path = dataset_learncurve_dir / 'learncurve-splits-metadata.json'
    logger.info(
        f"Loading learncurve splits from: {learncurve_splits_path}",
    )
    with learncurve_splits_path.open('r') as fp:
        learncurve_metadata = json.load(fp)
    ...
    for train_dur, replicate_dict in sorted(learncurve_metadata.items()):
        ...
        for replicate_num, splits_vectors_dict in sorted(
            ...
            split_csv_path = dataset_learncurve_dir / splits_vectors_dict["split_csv_filename"]
            logger.info(
            ...
            for window_dataset_kwarg in [
                ...
                vec_filename = splits_vectors_dict[window_dataset_kwarg]
